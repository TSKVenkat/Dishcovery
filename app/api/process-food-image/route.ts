...
export async function POST(request: Request) {
  try {
    ...
    // The image is already a base64 string, not a data URL at this point
    ...
    try {
      ...
      if (name === "NOT_FOOD" || name.toLowerCase().includes("cannot identify") || name.toLowerCase().includes("sorry")) {
        return NextResponse.json({ 
          name: "NOT_FOOD", 
